#!/bin/bash
# Ablation: same conversation, shuffled order. The context holds exactly the utterances
# the model would normally get, concatenated in a random order.
set -e
...
SAVING_PATH="./results/${DATASET_NAME}/${MODEL_SHORT_NAME}_t=${CONTEXT_DURATION}_shuffled_same_conversation__${DATETIME}"
...
python run_downstream_custom_multiple_fold.py \
    ...
    --context_source shuffled_same_conversation \
