#!/bin/bash
# Ablation: same session, random context. The context is a random mixture of utterances from
# the other conversations of the session.
set -e
...
SAVING_PATH="./results/${DATASET_NAME}/${MODEL_SHORT_NAME}_t=${CONTEXT_DURATION}_distant_scattered_session__${DATETIME}"
...
python run_downstream_custom_multiple_fold.py \
    ...
    --context_source distant_scattered_session \
